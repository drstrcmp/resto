...
class Login extends MY_Controller
{
    ...
    public function validasi()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $temp_user = $this->login_m->get_user($username);
            $num_user  = $temp_user->num_rows();
            ...
            if ($num_user == 0) {
                ...
                $num_account  = $temp_account->num_rows();

                if ($num_account > 0) {
                    $account_data = $temp_account->row();
                    $array_item = array(
                        'username'        => trim($account_data->user_username),
                        'pass'            => $account_data->user_password,
                        'nama'            => strtoupper(trim($account_data->user_name)),
                        'avatar'          => $account_data->user_avatar,
                        'level'           => $account_data->user_level,
